...
<div class="fragment fade-in">
Om vi vill jämföra två grupper kan vi använda ett **z-test (eller ett konfidensintervall) för två stickprov**
</div>
...
$$z = \frac{\hat p - p_0}{\sqrt {\frac{p \cdot (1-p)}{n}}} = \frac{0.25 - 0.3}{\sqrt{\frac{0.3 \cdot 0.7}{60}}} = -0.8452$$
</div>
...
<div style="float:right;">
```{r, echo = F, fig.align='center', fig.height=2, fig.width=4}
dat <- tibble(x = 0:10, y = dbinom(x, 10, 0.3), id = x < 5)
ggplot(dat, aes(x, y, fill = id)) + 
  ...
  theme(legend.position = "none", 
        ...
        plot.background = element_rect(fill = "#ace7ff", color = "#ace7ff")) +
  scale_x_continuous(breaks = c(0, 5, 10), labels = c(0, "x", 10))
```
</div>
